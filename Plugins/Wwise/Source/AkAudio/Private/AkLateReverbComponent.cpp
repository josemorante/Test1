...
void UAkLateReverbComponent::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	...
	AkSettings->OnShowReverbInfoChanged.Remove(ShowReverbInfoChangedHandle);
...
void UAkLateReverbComponent::UpdateTextVisibility()
{
	...
	if (GetDefault<UAkSettings>()->bShowReverbInfo && bEnable && GetWorld() != nullptr)
	{
...
{
	if (!GetDefault<UAkSettings>()->bShowReverbInfo) return;
	if (!IsTextVisualizersInitialized()) return;
